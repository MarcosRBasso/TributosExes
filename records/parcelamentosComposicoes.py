...
class parcelamentosComposicoes(Conexao):
    def __init__(self):
    ...
    def db_insert(self, idIntegracao, id_cloud, idParcelamentos, idParcelamentosAnt, idGuias, idDividas, idReceitas, valorReceita, valorCorrecao, valorJuro, 
        ...
        try:
            sql = """
                INSERT INTO parcelamentosComposicoes (                    
                    idIntegracao,                   
                    id_cloud, 
                    idParcelamentos,
                    idParcelamentosAnt,                                               
            ...
            self.commit()
            send_log_info(f"Agrupamentos {parcelamentosComposicoes} (id_cloud: {id_cloud}) inserido com sucesso.")
        except Exception as error:
            send_log_error(f"Erro ao inserir o anistias {parcelamentosComposicoes}. {error}")

    def db_delete(self):
        try:
            sql_s = f"SELECT * FROM parcelamentosComposicoes"
            if not self.query(sql_s):
                send_log_warning(f"parcelamentosComposicoes não encontrado para excluir.")
                return
            sql_d = f"DELETE FROM parcelamentosComposicoes WHERE id is not null"
            self.execute(sql_d)
            self.commit()
    ...
    def db_update(self, id, id_cloud, json, mensagem):
        try:
            sql_s = f"SELECT * FROM parcelamentosComposicoes WHERE id = {id}"
            if not self.query(sql_s):
                send_log_warning(f"atividades Economicas {id} não encontrado para atualizar.")
                return
            sql = """
                UPDATE 
                    parcelamentosComposicoes 
                SET 
                    id_cloud = %(id_cloud)s,
                    json_post = %(json)s,
    ...
    def db_search(self, id):
        try:
            sql = f"SELECT * FROM parcelamentosComposicoes WHERE id = {id}"
            data = self.query(sql)
            if data:
                return data
    ...
    def db_list(self):
        try:
            sql = "SELECT * FROM parcelamentosComposicoes WHERE id_cloud is null"
            data = self.query(sql)
            if data:
    ...
    def get_id_cloud(self, id):
        if (id == None):
            return None
        try:
            sql = f"SELECT id_cloud FROM parcelamentosComposicoes WHERE id_origem = {id}"
            data = self.query(sql)
            if data:
                return data[0][0]
    ...
    def send_post(self, id, idParcelamentos, idParcelamentosAnt, idGuias, idDividas, idReceitas, valorReceita, valorCorrecao, valorJuro, 
        ...
        objeto = {
            "idIntegracao": f"parcelamentosComposicoes{id}",
            "content": {}                                 
        }
        if idParcelamentos:
        ...
        envio = api_post("parcelamentosComposicoes", objeto)
...
parcelamentosComposicoes = parcelamentosComposicoes()