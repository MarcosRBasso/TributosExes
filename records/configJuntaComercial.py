...
class configParcCreditos(Conexao):
    ...
    def db_insert(self, idIntegracao, id_cloud, funcionalidadesJuntaComercial, tipoInteracaoEnviar, tipoInteracaoReceber):
        try:
            sql = """
                INSERT INTO configParcCreditos (                    
                    idIntegracao,                   
                    id_cloud,                                                           
                    funcionalidadesJuntaComercial, 
                    tipoInteracaoEnviar,
                    tipoInteracaoReceber                                             
                ) VALUES (
                    %(idIntegracao)s,                    
                    %(id_cloud)s,                                                            
                    %(funcionalidadesJuntaComercial)s,
                    %(tipoInteracaoEnviar)s,
                    %(tipoInteracaoReceber)s
                )
            """
            data = dict (
                idIntegracao = idIntegracao,
                id_cloud = id_cloud,                                                               
                funcionalidadesJuntaComercial = funcionalidadesJuntaComercial,                               
                tipoInteracaoEnviar = tipoInteracaoEnviar,
                tipoInteracaoReceber = tipoInteracaoReceber     
            )
    ...
    def send_post(self, id, funcionalidadesJuntaComercial, tipoInteracaoEnviar, tipoInteracaoReceber):
        objeto = {
            "idIntegracao": f"configParcCreditos{id}",
            "content": {}                                 
        }

        if tipoInteracaoEnviar != None:
            objeto[0]["content"]["CalculosTributariosAvancado"] = f"{tipoInteracaoEnviar}"    
               
        if funcionalidadesJuntaComercial != None:
            objeto[0]["camposadicionais"]["funcionalidadesJuntaComercial"] = f"{funcionalidadesJuntaComercial}"

        if tipoInteracaoReceber != None:
            objeto[0]["camposadicionais"]["tipoInteracaoReceber"] = f"{tipoInteracaoReceber}"        
